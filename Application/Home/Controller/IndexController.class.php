...
class IndexController extends BaseController{
    public function index(){
        ...
        if(((int)substr($hlist[0]['issue'],8,3))==103)
        {
            $nextissue=$t_history->where('State=1')->limit(7)->order('Id desc')->getField('Issue');
            $this->assign('nextissue',$nextissue);
        }
        else{
            $temp=$hlist[0]['issue'];
            $this->assign('nextissue',$temp+1);
        }
        //dump($list);

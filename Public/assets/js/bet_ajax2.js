...
$("#sub_btn").click(function(){
	...
	var nums=$("#bet_num").val().split(',').length;//下注数量
	var xze = parseInt($.trim($("#Money").val()).trim());
	var summoney =nums*xze;//下注总金额
	var umoney=parseFloat($("#money_span").text());//用户当前金额
	if(parseInt($("#JLFP").val())==0 || parseInt($("#firsttime").val())>600){
		...
		if(umoney<summoney){
			...
			popupalert(-1,"请选择下注号码和下注金额");
				...
				if (xze<tze || isNaN(xze)) {
					popupalert(-1,"请输入正确下注金额，最小下注额为"+tze);
				}
				else{
					var datas = {};
			        datas = { "issue": $("#DQQH").val(),
			        "bettype": $("#LX").val(),
			        "betnumber": $("#bet_num").val(),
			        "betmoney": $("#Money").val() ,
			        "peilv":$("#"+$("#LX").val()).val(),
			        "tze":$("#M_"+$("#LX").val()).val()
			    	};
    ...
    $.ajax({
        ...
        success: function (data) {
        	...
        	if (data.length>0) {
	        	...
	        	if(tabnum==1){
	        	 	$("#state_tab").html("");
	        	 	$("#state_footer_count").text("总共：0 条");
	        	 	$("#state_footer_money").text("总投注额：0");
	        	 }
	        	
	        }
	    },
        error: function (data, status) {
            popupalert(-1,"投注报表显示异常");
            //location.href='/Login/weblogin?state=other';
        }
    });
}
...
function firstopen(time){
	...
	if((h >= time2 && h <=23)||(h >= 0 && h <time1)){
    	sum = (10 - (m%10)-1)*60+(60 - s);
    }
    if(h >= time1 && h < time2){
    	sum = 25200 - ((h - 3)*3600 + m*60 + s);
    }
	$("#firsttime").val(sum);
}
...
setInterval(function(){
	...
	var num = parseInt($("#firsttime").val());

		if (num < 180 && num >=0) {
			if ($("#XQQH").val() == "" || $("#DQQH").val() == $("#XQQH").val()) {//没有获取到则，重新获取
				getTime();//获取数据库返回的时间
				// $("#XQQH").val();
			}
			if (num == 5) {
			...
			if (num == 0) {//时间为0时，重新倒计时赋值
				clearResult();//清空上期开奖结果
				if( parseInt($("#XQQH").val().substr(-2)) == 19 ){//下一期期号是19，则倒计时赋值为25200
					num = 25200;
				}
				else{
					num = 600;
				}
				$("#DQQH").val(parseInt($("#XQQH").val()));
				$(".DQQH").text(parseInt($("#XQQH").val()));
			}
		}
		if(num % 10 == 0){
			check_online();
	...
	if (num2 < 0) {num2 = 0}
	setResult(num);
	$("#kaijiang").text(parseInt(num));
	$("#firsttime").val(num)
	$("#fengpan").text(parseInt(num2));
	$("#JLFP").val(parseInt(num2));
},1000)
...
// var temp = 1;
function setResult(second){
	if(idata){
			if(second == 0){
				console.log(idata[0]["issue"]);
					var lastissue = parseInt(idata[0]["issue"]);
					$(".SQQH").text(lastissue);
			}
			if (second == 588 || second == 25188) {
